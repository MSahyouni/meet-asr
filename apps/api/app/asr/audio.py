...
def _ffmpeg_extract(src: str, dst_wav: str, target_sr=16000):
    import logging
    logger = logging.getLogger("ffmpeg_extract")
    
    try:
        logger.debug(f"FFmpeg: src={src}, dst={dst_wav}")
        ...
        if not os.path.exists(dst_wav):
            raise RuntimeError(f"FFmpeg output file not created: {dst_wav}")
        
        logger.debug(f"FFmpeg output: {dst_wav}")
